...
class UserNotificationControllerTest {
    ...
    @BeforeEach
    void setUp() {
        controller = new UserNotificationController(notificationService);
    }

    @Test
    void getUserNotificationsReturnsNotifications() {
        when(notificationService.getUserNotifications(6)).thenReturn(List.of(notification(1, 6)));

        ResponseEntity<List<Notification>> response = controller.getUserNotifications(6);
